...
import tempfile
#import amico.lut
import lut # to go back to previous version replace 'lut.' by 'amico.lut.'
...
class StickZeppelinBall( BaseModel ) :
    ...
    def generate( self, out_path, aux, idx_in, idx_out, ndirs ) :
        ...
        lm = lut.rotate_kernel( signal, aux, idx_in, idx_out, False, ndirs )
        ...
        for d in self.d_ISOs :
            ...
            lm = lut.rotate_kernel( signal, aux, idx_in, idx_out, True, ndirs )
            np.save( pjoin( out_path, 'A_%03d.npy'%progress.i ), lm )
            progress.update()
    ...
    def resample( self, in_path, idx_out, Ylm_out, doMergeB0, ndirs ) :
        ...
        KERNELS['wmr'][0,...] = lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, False, ndirs )[:,merge_idx]
        progress.update()
        ...
        for i in range(len(self.ICVFs)) :
            ...
            if lm.shape[0] != ndirs:
                ERROR( 'Outdated LUT. Call "generate_kernels( regenerate=True )" to update the LUT' )
            KERNELS['wmh'][i,...] = lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, False, ndirs )[:,merge_idx]
            progress.update()

        # Ball(s)
        for i in range(len(self.d_ISOs)) :
            lm = np.load( pjoin( in_path, 'A_%03d.npy'%progress.i ) )
            KERNELS['iso'][i,...] = lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, True, ndirs )[merge_idx]
            progress.update()
...
class CylinderZeppelinBall( BaseModel ) :
    ...
    def generate( self, out_path, aux, idx_in, idx_out, ndirs ) :
        if self.scheme.version < 1 :
            ERROR( 'This model requires a "VERSION: STEJSKALTANNER" scheme or a generalized scheme' )
        else:
            ...
            if self.scheme.version == 1 :
                ...
                scheme_high.write_Camino_schemefile('temp_scheme.txt')
                file_with_point = open('temp_scheme.txt', 'r')
                file_with_comma = open(filename_scheme, 'w')
                lines = file_with_point.readlines()
                for line in lines:
                    file_with_comma.write(",".join(line.split(".")))
                file_with_point.close()
                file_with_comma.close()   
                filename_scheme_version1 = pjoin( out_path, 'scheme_version1.txt' )
                scheme_version1 = scheme_high.to_version_1()
                np.savetxt( filename_scheme_version1, scheme_version1.raw, fmt='%15.8e', delimiter=' ', header='VERSION: STEJSKALTANNER', comments='' )
    ...
    def resample( self, in_path, idx_out, Ylm_out, doMergeB0, ndirs ) :
        ...
        for i in range(len(self.Rs)) :
            ...
            if lm.shape[0] != ndirs:
                ERROR( 'Outdated LUT. Call "generate_kernels( regenerate=True )" to update the LUT' )
            KERNELS['wmr'][i,:,:] = lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, False, ndirs )[:,merge_idx]
            progress.update()
    ...
    def fit( self, y, dirs, KERNELS, params, htable ) :
        ...
        o = 0
        for i in range(nD) :
            lut_idx = lut.dir_TO_lut_idx( dirs[i], htable )
...
class NODDI( BaseModel ) :
    ...
    def resample( self, in_path, idx_out, Ylm_out, doMergeB0, ndirs ):
        ...
        for i in range( len(self.IC_ODs) ):
            for j in range( len(self.IC_VFs) ):
                ...
                if lm.shape[0] != ndirs:
                    ERROR( 'Outdated LUT. Call "generate_kernels( regenerate=True )" to update the LUT' )
                idx = progress.i - 1
                KERNELS['wm'][idx,:,:] = lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, False, ndirs )[:,merge_idx]
        ...
        # Isotropic
        lm = np.load( pjoin( in_path, 'A_%03d.npy'%progress.i ) )
        KERNELS['iso'] = lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, True, ndirs )[merge_idx]
        progress.update()
    ...
    def fit( self, y, dirs, KERNELS, params, htable ) :
        ...
        if self.isExvivo == True :
            nATOMS += 1
        lut_idx = lut.dir_TO_lut_idx( dirs[0], htable )
        A = np.ones( (len(y), nATOMS), dtype=np.float64, order='F' )
...
class FreeWater( BaseModel ) :
    ...
    def resample( self, in_path, idx_out, Ylm_out, doMergeB0, ndirs ) :
        ...
        for i in range(len(self.d_perps)) :
            ...
            if lm.shape[0] != ndirs:
                ERROR( 'Outdated LUT. Call "generate_kernels( regenerate=True )" to update the LUT' )
            KERNELS['D'][i,...] = lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, False, ndirs )[:,merge_idx]
            progress.update()

        # Isotropic compartment(s)
        for i in range(len(self.d_isos)) :
            lm = np.load( pjoin( in_path, 'A_%03d.npy'%progress.i ) )
            KERNELS['CSF'][i,...] = lut.resample_kernel( lm, self.scheme.nS, idx_out, Ylm_out, True, ndirs )[merge_idx]
            progress.update()
